...
namespace CachingEventProcessorHostWebJob
{
    public class CachingProcessor : IEventProcessor
    {
        ...
        Task IEventProcessor.OpenAsync(PartitionContext context)
        {
            ...
            Console.WriteLine($"CachingProcessor initialized. Partition '{context.Lease.PartitionId}', Offset '{context.Lease.Offset}'");
        ...
        async Task IEventProcessor.ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
        {
            try
            {
                ...
                if (_checkpointStopWatch.Elapsed > TimeSpan.FromSeconds(3))
                {
                    ...
                    Console.WriteLine($"Checkpoint partition {context.Lease.PartitionId} progress.");
